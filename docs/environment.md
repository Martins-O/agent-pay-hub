...
- `SOLANA_PAYER_SECRET`: Base58-encoded secret key for the server-controlled payer (required when `SOLANA_SIMULATION_ONLY` is `false` and no signer list is provided).
- `SOLANA_PAYER_SECRETS`: JSON array or comma-separated list of additional payer secrets used for signer rotation. When present, the server round-robins these wallets; at least one secret (via either variable) is required for real submissions.
